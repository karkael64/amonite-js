...
class Amonite extends EventTarget {
    ...
    configure(next) {
        ...
        if (len) {
            let incr = 0,
                has_error = false,
                id = setTimeout(() => {
                    next(new Error('Configuration is time out.'))
                }, 1000),
                args = [
                    this.req,
                    this.res,
    ...
    getController(next) {
        ...
        if (len) {
            let id = setTimeout(() => {
                    next(new Error('Controller selection is time out.'))
                }, 1000),
                done = false,
                incr = 0,
                args = [
    ...
    getAnswer(controller, next) {
        ...
        let id = setTimeout(() => {
            next(new Error('Controller answer is time out.'));
        }, 1000);
        try {
                    ...
                    if (is_function(answer))
                        return answer(this.req, this.res, (err, answer) => {
                            if (err) {
                                ...
                                if (is_string(answer)) return next(null, answer);
